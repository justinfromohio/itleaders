...
function archer_editor_data($value) {
...
function archer_get_option($k, $v = '', $m = 'theme-settings') {
    if (defined('FW')) {
        switch ($m) {
            case 'theme-settings':
...
function archer_get_image($k, $v = '', $d = false) {

    if ($d == true) {
        $attachment = $k;
    } else {
        $attachment = archer_get_option($k);
    }
 ...
 * sience 1.0
 * archer_image($img, $alt )
 */

function archer_image($img, $alt, $v = '') {
...
function archer_main($id, $name = true) {
...
// since 2.0
function archer_sectionID($id, $returnID = false) {
...
function archer_get_post_meta($id, $needle) {
    $data = get_post_meta($id, 'fw_options');
    if (is_array($data) && isset($data[0]['page_sections'])) {
        $data = $data[0]['page_sections'];

        if (is_array($data)) {
            return archer_seekKey($data, $needle);
        }
    }
}

function archer_seekKey($haystack, $needle) {
    foreach ($haystack as $key => $value) {

        if ($key == $needle) {
            return $value;
        } elseif (is_array($value)) {
            return archer_seekKey($value, $needle);
        }
    }
}
...
if (!function_exists('archer_theme_button_class')) :

    function archer_theme_button_class($style) {
        /**
        ...
        if ($style == 'default') {
            ...
            echo 'btn btn-primmary';
        } elseif ($style == 'download') {
            echo 'btn btn-download';
        } elseif ($style == 'learn_more') {
            echo 'learn-more-link';
        }
        else {
            echo 'default';
        }
...
if (!function_exists('archer_theme_translate')) :

    function archer_theme_translate($content) {
        /**
...
// term
if (!function_exists('archer_get_category_term_list')) :

    function archer_get_category_term_list() {
        /**
        ...
        $result = array();
        $result[0] = esc_html__('All Categories', 'archer');
...
if (!function_exists('archer_get_header_icons')) {

    function archer_get_header_icons() {
        ...
        $meta_tags = array(
            sprintf('<link rel="icon" href="%s" sizes="32x32" />', esc_url(archer_get_image('favicon', ARCHER_IMAGES . '/ico/favicon.png'))),
            sprintf('<link rel="icon" href="%s" sizes="192x192" />', esc_url(archer_get_image('site_icon_192', ARCHER_IMAGES . '/ico/site-icon-192.png'))),
            sprintf('<link rel="apple-touch-icon-precomposed" href="%s" />', esc_url(archer_get_image('site_icon_180', ARCHER_IMAGES . '/ico/site-icon-180.png'))),
            sprintf('<meta name="msapplication-TileImage" content="%s" />', esc_url(archer_get_image('site_icon_270', ARCHER_IMAGES . '/ico/site-icon-270.png'))),
        );
 ...
 */
if (!function_exists('archer_prerloader')) {

    function archer_prerloader() {
        if (defined("FW")) {
            ?>
...
function archer_edit_section() {
    ?>
    <div class="section-edit">
        <div class="container relative">
    <?php
    if (is_user_logged_in()) {
        edit_post_link(esc_html__('Edit', 'archer'), '', '');
    }
    ?>
