...
class Wellnesscenter_Option_Type_Table extends FW_Option_Type
{
	...
	protected function _render( $id, $option, $data ) {
		...
		if ( ! $table_shortcode ) {
			trigger_error(
				__( 'table-builder option type must be inside the table shortcode', 'archer' ),
				E_USER_ERROR
			);
		}
	...
	protected function _get_defaults() {
		return array(
			'header_options'  => array(
				'table_purpose' => array(
					'type'    => 'select',
					'label'   => __( 'Table Styling', 'archer' ),
					'desc'    => __( 'Choose the table styling options', 'archer' ),
					'choices' => array(
						'pricing' => __( 'Use the table as a pricing table', 'archer' ),
						'tabular' => __( 'Use the table to display tabular data', 'archer' ),
					),
					'value'   => 'pricing',
					'attr'    => array(
			...
			'row_options'     => array(
				'name' => array(
					...
					'desc'    => false,
					'choices' => array(
						'default-row' => __( 'Default row', 'archer' ),
						'heading-row' => __( 'Heading row', 'archer' ),
						'pricing-row' => __( 'Pricing row', 'archer' ),
						'button-row'  => __( 'Button row', 'archer' ),
						'switch-row'  => __( 'Modal button row', 'archer' )
					),
				)
			),
			'columns_options' => array(
				'name' => array(
					...
					'desc'    => false,
					'choices' => array(
						'default-col'   => __( 'Default column', 'archer' ),
						'highlight-col' => __( 'Highlight column', 'archer' ),
					),
				)
			),
			'content_options' => array(
				...
				'switch-row' => array(
					'button_text' => array(
						...
						'desc'  => false,
						'attr' => array(
							'placeholder' => __('button text', 'archer')
						),
					),
				),
				'heading-row' => array(
					...
					'sub_heading' => array(
						...
						'desc'  => false,
						'attr' => array(
							'placeholder' => __('sub title (can be left blank)', 'archer')
						),
					),
				),
				'pricing-row' => array(
				...
				'button-row'  => array(
					'button' => ($button = fw()->extensions->get( 'shortcodes' )->get_shortcode( 'button' ))
						? array(
							'type'          => 'popup',
							'popup-title'   => __( 'Button', 'archer' ),
							'button'        => __( 'Add', 'archer' ),
