...
//fw_print($atts['height']);

//$frame = $atts['frame'];esc_attr($frame). ' ' . 
?>
<div class="fw-single-image <?php echo esc_attr( $position ); ?>" <?php echo  $style; ?>>
	<?php if ( empty( $atts['link'] ) ) : 
            $height = $atts['height'] ? "style='height: {$atts['height']}px'" : '';
            ?>
    <img src="<?php echo esc_url( $image['url'] ) ?>" class="img-responsive" <?php echo $height ?> alt=""  />
	<?php else : ?>
